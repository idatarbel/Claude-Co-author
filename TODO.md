...
- [In Progress] 4. **Fix "context.document.getComments is not a function"** — `getComments()` lives on `Word.Body`, not on `Word.Document`. Fix both call sites in Code.js (readDocState + replyToComment) to use `context.document.body.getComments()`.
